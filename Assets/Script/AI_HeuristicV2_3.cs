...
public class AI_HeuristicV2_3 : MonoBehaviour {
	...
	int discordCount = 0;
	int deckCount = 80;
	int[,] dis_function_count = new int[5,17];
...
*/
	...
	public Card Card(){
		...
		for (int i=0; i<hands.Count; i++) {
			...
			if(!_isLegal(hands[i],_GetNowCard())){
				cardPriority[i] = 0;
				//Debug.Log(hands[i].color.ToString() +  hands[i].function + " notlegal");
				continue;
			}
			cardPriority[i] = search(hands, hands[i],2);
	...
	void RerecordDiscord(Card newcard){
		if (newcard.color == global::Card.CardColor.Red)
			dis_color_count_red--;
		else if (newcard.color == global::Card.CardColor.Yellow)
			dis_color_count_yellow--;
	...
	bool _isLegal(Card card,Card tnowcard){
		//Debug.Log(card.color.ToString() +  card.function + " g");
		if ((card.color != tnowcard.color) & (card.function != tnowcard.function)) {
